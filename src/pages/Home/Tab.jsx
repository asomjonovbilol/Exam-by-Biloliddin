...
const Tabs = () => {
  ...
  const chosenCategory = useSelector((state) => state.filterSlice.category);
  ...
  function handleFilter() {
    let filtered = JSON_DATA.filter(
      (product) => product.price >= priceStart && product.price <= priceEnd
    );
    if (chosenCategory != "all") {
      filtered = filtered.filter((product) =>
        product.categories.some((category) => category == chosenCategory)
      );
    }
    ...
    <div className="mt-10">
      <Box sx={{ width: "100%", typography: "body1" }}>
        <TabContext value={value}>
          <div>
            <div className="flex items-center xl:flex-col ml-4 justify-between gap-6">
              <Box
                sx={{
                  borderBottom: 1,
                  borderColor: "white",
            ...
            <div>
              <TabPanel value="1">
                <CardProducts value={value} filteredProducts={filteredProducts} />
              </TabPanel>
              <TabPanel value="2">
                <CardProducts value={value} filteredProducts={filteredProducts} />
              </TabPanel>
